...
const colors = [ '#A7C5C9', '#546F72', '#8EDCE6', '#F4A597', '#E6EDEB', '#C9DDD8', '#FFEFEF', '#969E8E' ];

const changeColor = () => {
	const generateColors = Math.floor(Math.random() * colors.length);
	console.log(generateColors);
	return colors[generateColors];
};
section2.addEventListener('mousemove', () => {
	section2.style.backgroundColor = changeColor();
	section2.style.transition = '1s';
});
